module ActiveRecord
  module QueryMethods
    class WhereChain
      ...
      def contains(opts, *rest)
        build_where_chain(opts, rest) do |rel|
          ...
          when Arel::Nodes::In, Arel::Nodes::Equality
            column = rel.left.relation.engine.columns.find do |col|
              col.name == rel.left.name.to_s || col.name == rel.left.relation.name.to_s
            end
            
            if !column
              if assoc = column_from_related_table(rel)
                column = assoc.klass.columns.find do |col|
                  col.name == rel.left.name.to_s
                end
              end
            elsif column.type == :hstore
              new_right_value = if rel.right.respond_to?(:val)
                            rel.right.val
                          else
                            rel.right
                          end
              ...
              rel.right = if Arel::Nodes.respond_to?(:build_quoted)
                            Arel::Nodes.build_quoted({new_right_name => new_right_value},
                                                     rel.left)
                          else
                           { new_right_name => new_right_value }
                          end
            end
            
            if column && column.type == :hstore
              Arel::Nodes::ContainsHStore.new(rel.left, rel.right)
            elsif column && (column.respond_to?(:array) && column.array)
      ...
      def column_from_related_table(rel)
